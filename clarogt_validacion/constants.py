FIXED_HEAD_SLICE = slice(0, 7)
FIXED_SECTION_SLICE = slice(0, 2)
FIXED_SUB_SECTION_SLICE = slice(2, 4)
FIXED_SUB_SUB_SECTION_SLICE = slice(4, 7)
FIXED_PREDICATE_SLICE = slice(7, None)
FIXED_ENDING_SECTION = 50


MOBILE_HEAD_SLICE = slice(0, 6)
MOBILE_SECTION_SLICE = slice(0, 3)
MOBILE_SUB_SECTION_SLICE = slice(3, 4)
MOBILE_SUB_SUB_SECTION_SLICE = slice(4, 6)
MOBILE_PREDICATE_SLICE = slice(6, None)
MOBILE_ENDING_SECTION = 999

BAD_SUFFIXES = [
    ".bad",
    ".er",
    ".err",
    ".error",
]